...
void PointerInputTest::testWarpingGeneratesPointerMotion()
{
    ...
    QSignalSpy warpedSpy(pointer.get(), &Test::WlPointer::warp);
    ...
    QVERIFY(warpedSpy.wait());
    QCOMPARE(warpedSpy.count(), 1);
    QCOMPARE(warpedSpy.last().first().toPointF(), QPointF(26, 26));
}
