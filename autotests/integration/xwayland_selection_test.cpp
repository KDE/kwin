...
private Q_SLOTS:
    ...
    void waylandPrimarySelectionChangesBeforeTargets();
...
void XwaylandSelectionTest::waylandClipboardChangesBeforeTargets()
{
    ...
    QVERIFY(waylandServerSeatSelectionChangedSpy.wait());
...
void XwaylandSelectionTest::waylandPrimarySelectionChangesBeforeTargets()
{
    // If an X11 client claims a selection, the data bridge code will not set a data source immediately.
    // Instead, it'll ask the X11 client for TARGETS. The reason for that is that wayland clients
    // expect mime types to be announced with the wl_data_offer. This test verifies that if the wayland
    // primary selection changes before a TARGETS reply arrives, the X11 clients will still be able to
    // get the primary selection data set by the Wayland client.
    ...
    QSignalSpy waylandServerSeatPrimarySelectionChangedSpy(waylandServer()->seat(), &SeatInterface::primarySelectionChanged);

    // When KWin requests TARGETS from the X11 owner, an alternative selection appears
    std::unique_ptr<TestDataSource> overrideSource;
    x11Selection->setOnTargets([&]() {
        overrideSource = std::make_unique<TestDataSource>(QHash<QString, QByteArray>{{QStringLiteral("text/plain"), QByteArrayLiteral("test data source")}});
        waylandServer()->seat()->setPrimarySelection(overrideSource.get(), waylandServer()->display()->nextSerial());
    });
    x11Selection->setOwner(true);

    QVERIFY(waylandServerSeatPrimarySelectionChangedSpy.wait());
    QCOMPARE(waylandServer()->seat()->primarySelection(), overrideSource.get());
    const QByteArray actualData = X11SelectionReader::read(x11Display.get(), x11Window->window(), atoms->primary, x11Display->mimeTypeToAtom(plainText), atoms->wl_selection, XCB_CURRENT_TIME);
    QCOMPARE(actualData, "test data source");
}
