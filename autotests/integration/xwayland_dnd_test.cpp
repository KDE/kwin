...
public:
    ...
    virtual bool handle(xcb_generic_event_t *event) = 0;
...
public:
    ...
    xcb_atom_t mimeTypeToAtom(const QString &mimeType) const
    {
        const QByteArray utf8MimeType = mimeType.toUtf8();
        xcb_intern_atom_cookie_t cookie = xcb_intern_atom(m_connection.get(), 0, utf8MimeType.size(), utf8MimeType.data());
    ...
    QString atomToMimeType(xcb_atom_t atom) const
    {
...
private:
    ...
    void dispatchEvents()
    {
        ...
        while ((event = xcb_poll_for_event(m_connection.get()))) {
            for (X11Object *object : std::as_const(m_objects)) {
...
public:
    explicit X11SelectionOwner(X11Display *display, xcb_atom_t selection, const QStringList &mimeTypes, std::function<X11SelectionData(const QString &mimeType)> renderFunction)
        : X11Object(display)
    ...
    bool handle(xcb_generic_event_t *event) override
    {
        ...
        case XCB_SELECTION_NOTIFY:
            onSelectionNotify(reinterpret_cast<xcb_selection_notify_event_t *>(event));
            break;
...
private:
    void onSelectionNotify(xcb_selection_notify_event_t *event)
    {
    }
    ...
    void sendTargets(xcb_window_t requestor, xcb_atom_t property)
    {
        ...
        xcb_change_property(m_display->connection(), XCB_PROP_MODE_REPLACE, requestor, property, XCB_ATOM_ATOM, 32, sizeof(targets) / sizeof(*targets), targets);
    }
    ...
    {
        const QString mimeType = m_display->atomToMimeType(target);
    ...
    std::function<X11SelectionData(const QString &mimeType)> m_renderFunction;
...
public:
    X11SelectionReader(X11Display *display, xcb_window_t requestor, xcb_atom_t selection, xcb_atom_t target, xcb_atom_t property)
        : X11Object(display)
        , m_requestor(requestor)
        , m_selection(selection)
    {
        xcb_convert_selection(display->connection(), requestor, selection, target, property, display->time());
    }
    ...
    bool handle(xcb_generic_event_t *event) override
    {
        if (XCB_EVENT_RESPONSE_TYPE(event) == XCB_SELECTION_NOTIFY) {
            ...
            if (selectionNotifyEvent->requestor == m_requestor && selectionNotifyEvent->selection == m_selection) {
                Q_EMIT done(readBlock(selectionNotifyEvent->requestor, selectionNotifyEvent->property));
            }
        }
        return false;
    }

    static QByteArray read(X11Display *display, xcb_window_t requestor, xcb_atom_t selection, xcb_atom_t target, xcb_atom_t property)
    {
        X11SelectionReader reader(display, requestor, selection, target, property);
...
private:
    QByteArray readBlock(xcb_window_t window, xcb_atom_t property) const
    {
...
public Q_SLOTS:
    ...
    void sendEnter(xcb_window_t target, const QStringList &mimeTypes)
    {
...
public:
    ...
    bool handle(xcb_generic_event_t *event) override
    {
        ...
        case XCB_MOTION_NOTIFY: {
            const auto motionEvent = reinterpret_cast<xcb_motion_notify_event_t *>(event);
            Q_EMIT motion(motionEvent->event, QPoint(motionEvent->root_x, motionEvent->root_y));
            break;
        }
        case XCB_BUTTON_PRESS: {
            const auto buttonEvent = reinterpret_cast<xcb_button_press_event_t *>(event);
            Q_EMIT pressed(buttonEvent->event, buttonEvent->detail, QPoint(buttonEvent->root_x, buttonEvent->root_y));
            break;
        }
        case XCB_BUTTON_RELEASE: {
            const auto buttonEvent = reinterpret_cast<xcb_button_release_event_t *>(event);
            Q_EMIT released(buttonEvent->event, buttonEvent->detail);
            break;
        }
        }
        return false;
...
Q_SIGNALS:
    ...
    void motion(xcb_window_t window, const QPointF &position);
    void pressed(xcb_window_t window, xcb_button_t button, const QPoint &position);
    void released(xcb_window_t window, xcb_button_t button);
};
...
void XwaylandDndTest::x11ToX11()
{
    ...
    // Xwayland keeps the pointer in the top-left corner by default, move it elsewhere so it doesn't interfere with enteredSpy.
    xcb_warp_pointer(x11Display->connection(), XCB_WINDOW_NONE, x11Display->root(), 0, 0, 0, 0, 640, 512);
    ...
    X11SelectionOwner x11DragSelection(x11Display.get(), atoms->xdnd_selection, {QStringLiteral("text/plain")}, [](const QString &mimeType) {
    ...
    x11Drag->move(pressedSpy.last().at(2).value<QPoint>());
    ...
    // Ask for data.
    const QByteArray data = X11SelectionReader::read(x11Display.get(), targetWindow->window(), atoms->xdnd_selection, x11Display->mimeTypeToAtom(QStringLiteral("text/plain")), atoms->wl_selection);
...
void XwaylandDndTest::x11ToWayland()
{
    ...
    // Accept the offer.
    auto offer = waylandDataDevice->dragOffer();
    ...
    offer->accept(QStringLiteral("text/plain"), waylandDataDeviceDragEnteredSpy.last().at(0).value<quint32>());
    ...
    // Ask for data.
    const QFuture<QByteArray> data = readMimeTypeData(offer, QStringLiteral("text/plain"));
    QVERIFY(waitFuture(data));
    QCOMPARE(data.result(), QByteArrayLiteral("x11 -> wayland"));

    // Finish drag and drop.
    QSignalSpy x11DragFinishedSpy(x11Drag.get(), &X11Drag::finished);
    offer->dragAndDropFinished();
    QEXPECT_FAIL("", "XdndFinished event is not synchronized to wl_data_offer.finish", Continue);
    QVERIFY(x11DragFinishedSpy.wait(100));
}
...
void XwaylandDndTest::destroyX11ToWaylandSource()
{
    ...
    QSignalSpy dragStartedSpy(waylandServer()->seat(), &SeatInterface::dragStarted);
    auto x11DragSelection = std::make_unique<X11SelectionOwner>(x11Display.get(), atoms->xdnd_selection, QStringList{QStringLiteral("text/plain")}, [](const QString &mimeType) {
        if (mimeType == QLatin1String("text/plain")) {
            return X11SelectionData{
                .data = QByteArrayLiteral("x11 -> wayland"),
                .type = atoms->text,
...
void XwaylandDndTest::waylandToX11()
{
    ...
    // Ask for data.
    const QByteArray data = X11SelectionReader::read(x11Display.get(), x11Window->window(), atoms->xdnd_selection, x11Display->mimeTypeToAtom(QStringLiteral("text/plain")), atoms->wl_selection);
    QCOMPARE(data, QByteArrayLiteral("wayland -> x11"));
...
{
    // This test verifies that the data bridge doesn't consider XDND unaware window as drop targets.
