...
static QMap<int, Rect> enumerateRects(int xSize, int ySize, const QSize &unit = QSize(5, 5))
{
    QList<int> sumAreaTable;
    sumAreaTable.resize((xSize + 1) * (ySize + 1));

    const int satStride = xSize + 1;
    ...
    for (int y = 0; y < ySize; ++y) {
        ...
        for (int x = 0; x < xSize; ++x) {
            sumAreaTable[(y + 1) * satStride + x + 1] = sumAreaTable[(y + 1) * satStride + x]
                - sumAreaTable[y * satStride + x] + sumAreaTable[y * satStride + x + 1] + xyToPow2(x, y, xSize);
        }
    }
    ...
    for (int y1 = 0; y1 < ySize; ++y1) {
        for (int y2 = y1 + 1; y2 <= ySize; ++y2) {
            for (int x1 = 0; x1 < xSize; ++x1) {
                for (int x2 = x1 + 1; x2 <= xSize; ++x2) {
 ...
 */
static Region bitsToRegion(int bits, int xSize, int ySize, const QSize &unit = QSize(5, 5))
{
    QList<Rect> rects;
    rects.reserve(xSize * ySize);
...
static QList<Region> enumerateRegions(int xSize, int ySize, const QSize &unit = QSize(5, 5))
{
    const int maxVariations = std::pow(2, xSize * ySize);

    QList<Region> regions;
    regions.reserve(maxVariations);
    for (int i = 0; i < maxVariations; ++i) {
        regions.append(bitsToRegion(i, xSize, ySize, unit));
    }
...
private Q_SLOTS:
    ...
    void translated();
    void scaledAndRounded_data();
    void scaledAndRounded();
...
private:
    const int xResolution = 3;
    const int yResolution = 3;

    const QList<Region> m_regions = enumerateRegions(xResolution, yResolution);
    const QMap<int, Rect> m_rects = enumerateRects(xResolution, yResolution);
};
...
void TestRegion::containsRect()
{
    for (int i = 0; i < m_regions.size(); ++i) {
        for (const auto &[j, rect] : m_rects.asKeyValueRange()) {
            QCOMPARE(m_regions[i].contains(rect), (i & j) == j);
        }
    }
}
...
void TestRegion::scaledAndRounded_data()
{
    QTest::addColumn<Region>("region");
    ...
    QTest::addRow("simple @ 1.25") << simple << 1.25 << Region(1, 3, 4, 5);
    QTest::addRow("simple @ 1.5") << simple << 1.5 << Region(2, 3, 4, 6);
    QTest::addRow("simple @ 1.75") << simple << 1.75 << Region(2, 4, 5, 7);
    ...
    QTest::addRow("complex @ 1.25") << complex << 1.25 << (Region(1, 3, 4, 5) | Region(6, 8, 9, 10));
    QTest::addRow("complex @ 1.5") << complex << 1.5 << (Region(2, 3, 4, 6) | Region(8, 9, 10, 12));
    QTest::addRow("complex @ 1.75") << complex << 1.75 << (Region(2, 4, 5, 7) | Region(9, 11, 12, 14));
    QTest::addRow("complex @ 2.0") << complex << 2.0 << (Region(2, 4, 6, 8) | Region(10, 12, 14, 16));
}

void TestRegion::scaledAndRounded()
{
    QFETCH(Region, region);
    QFETCH(qreal, scale);
    QTEST(region.scaledAndRounded(scale), "expected");
}
