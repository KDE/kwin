...
void GLShader::resolveLocations()
{
    ...
    for (int i = 0; i < maxUniformCount; i++) {
        int location = uniformLocation(std::format("colorPipelineClamp{}", i).c_str());
        if (location >= 0) {
            m_pipelineUniforms.clamps.push_back(location);
        } else {
            break;
        }
    }
...
void GLShader::setColorspaceUniforms(const std::shared_ptr<ColorDescription> &src, const std::shared_ptr<ColorDescription> &dst, RenderingIntent intent)
{
    ...
    if (src->transferFunction().type == TransferFunction::BT1886) {
        ...
        setUniform(Vec2Uniform::SourceTransferFunctionParams, QVector2D(src->transferFunction().hlgBeta(), src->transferFunction().maxLuminance));
    } else {
    ...
    if (dst->transferFunction().type == TransferFunction::BT1886) {
        ...
        setUniform(Vec2Uniform::DestinationTransferFunctionParams, QVector2D(dst->transferFunction().hlgBeta(), dst->transferFunction().maxLuminance));
    } else {
...
void GLShader::setColorPipeline(const ColorPipeline &pipeline)
{
    ...
    for (const auto &op : pipeline.ops) {
        ...
        if (const auto mat = std::get_if<ColorMatrix>(&operation)) {
            ...
            lut3DCount++;
            textureIndex++;
        } else if (const auto clamp = std::get_if<ColorClamp>(&operation)) {
            setUniform(m_pipelineUniforms.clamps[clampCount], QVector2D(clamp->m_minValue, clamp->m_maxValue));
            clampCount++;
        } else {
            Q_UNREACHABLE();
        }
