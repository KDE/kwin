                                                                                                  ...
                                                                                                  const std::shared_ptr<ColorDescription> &color)
{
    ...
    if (m_lastFormat != buffer->dmabufAttributes()->format || !m_swapchain) {
        ...
        auto format = chooseFormat(buffer->dmabufAttributes()->format, m_sourceContext->displayObject()->allSupportedDrmFormats(),
                                   m_importContext->displayObject()->allSupportedDrmFormats());
        if (format) {
            m_swapchain = EglSwapchain::create(m_allocator, m_sourceContext.get(), buffer->size(), format->format, format->modifiers);
        } else {
            // TODO implement (and benchmark) options for this code path:
    ...
    if (!conv) {
        ...
        // TODO when possible, use a blit instead of a shader for better efficiency
        ...
        // TODO oof, this whole context switching mess will require being able
