...
std::unique_ptr<MultiGpuSwapchain> MultiGpuSwapchain::createForSampling(RenderDevice *sourceDevice, DrmDevice *targetDevice, uint32_t format, uint64_t modifier, const QSize &size, const FormatModifierMap &importFormats)
{
    ...
    if (modifier == DRM_FORMAT_MOD_INVALID) {
        ...
        auto firstCopy = createCopy(sourceDevice, format, {modifier}, sysMemDevice->allocator(), size, destination->allImportableFormats(), false);
        if (!firstCopy) {
            return nullptr;
        }
        ...
        auto firstCopy = createCopy(sourceDevice, format, {modifier}, sysMemDevice->allocator(), size, importFormats, false);
        if (!firstCopy) {
            return nullptr;
        }
                                                                                     ...
                                                                                     const QSize &size, const FormatModifierMap &importFormats)
{
    ...
    RenderDevice *sysMemDevice = GpuManager::self()->softwareDevice() ? GpuManager::self()->softwareDevice() : sourceDevice;
    auto firstCopy = createCopy(sourceDevice, format, modifiers, sysMemDevice->allocator(), size, destination->allImportableFormats(), false);
    if (!firstCopy) {
        return std::nullopt;
    }
