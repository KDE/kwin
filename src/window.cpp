    ...
    , m_colorScheme(QStringLiteral("kdeglobals"))
{
    ...
    connect(this, &Window::frameGeometryChanged, this, [this](Window *c, const QRectF &old) {
        ...
        if (isOnScreenDisplay() && !frameGeometry().isEmpty() && old.size() != frameGeometry().size() && isPlaceable()) {
            ...
            Placement::self()->place(this, workspace()->clientArea(PlacementArea, this, workspace()->activeOutput()).toRect());
        }
    });
...
void Window::getWmOpaqueRegion()
{
    ...
    for (const auto &r : rects) {
        // DAVE - X to native? adn why is in this in window
...
QRegion Window::shapeRegion() const
{
    ...
    if (shape()) {
        ...
        if (!reply.isNull()) {
            ...
            // Dave, X to native - also why is in this in window?
        } else {
            m_shapeRegion = QRegion();
        }
    } else {
                    ...
                    if (screens()->count() > 1) { // optimization
                        ...
                        for (const QRectF &r : strut) {
                            if (r.top() == 0 && r.width() > r.height() && // "top panel"
                                ...
                                newTitleTop = r.bottom() /*+ 1*/;//DAVE is this -1 an old qrect hack?
                                break;
                            }
                        }
                        if (newTitleTop > -1) {
