...
void Window::handleInteractiveMoveResize(const QPointF &local, const QPointF &global)
{
    ...
    if (isInteractiveResize()) {
        ...
        if (nextMoveResizeGeom != currentMoveResizeGeom) {
            ...
            if (!isRequestedFullScreen()) {
                if (maximizeMode() != MaximizeRestore || requestedMaximizeMode() != MaximizeRestore) {
                    maximize(MaximizeRestore);
                    return;
...
QRectF Window::nextInteractiveMoveGeometry(const QPointF &global) const
{
    ...
    const QPointF topleft = global - QPointF(interactiveMoveOffset().x() * width(), interactiveMoveOffset().y() * height());

    // first move, then snap, then check bounds
    const QRectF currentMoveResizeGeom = frameGeometry();
    QRectF nextMoveResizeGeom = frameGeometry();
    nextMoveResizeGeom.moveTopLeft(topleft);
...
bool Window::performMouseCommand(Options::MouseCommand cmd, const QPointF &globalPos)
{
    ...
    case Options::MouseUnrestrictedMove: {
        ...
        setInteractiveMoveOffset(QPointF((globalPos.x() - x()) / width(), (globalPos.y() - y()) / height())); // map from global
...
bool Window::processDecorationButtonPress(QMouseEvent *event, bool ignoreMenu)
{
        ...
        && com != Options::MouseMinimize) // mouse release event
    {
        setInteractiveMoveResizeGravity(mouseGravity());
        setInteractiveMoveResizePointerButtonDown(true);
        setInteractiveMoveOffset(QPointF(event->pos().x() / width(), event->pos().y() / height()));
