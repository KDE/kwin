...
void Window::handleInteractiveMoveResize(const QPointF &local, const QPointF &global)
{
    ...
    if (!isRequestedFullScreen() && isInteractiveMove()) {
        ...
        if (input()->modifiersRelevantForGlobalShortcuts() & Qt::ShiftModifier && TileManager::instance(output(), VirtualDesktopManager::self()->currentDesktop(), workspace()->activities()->current())->rootTile()->childCount() > 1) {
...
QRectF Window::quickTileGeometry(QuickTileMode mode, const QPointF &pos) const
{
    ...
    if (mode & QuickTileFlag::Custom) {
        Tile *tile = TileManager::instance(out, VirtualDesktopManager::self()->currentDesktop(), workspace()->activities()->current())->bestTileForPosition(pos);
        if (tile) {
    ...
    Tile *tile = TileManager::instance(out, VirtualDesktopManager::self()->currentDesktop(), workspace()->activities()->current())->quickTile(mode);
    if (tile) {
        return tile->windowGeometry();
    }
...
void Window::setQuickTileMode(QuickTileMode mode, bool keyboard)
{
    ...
    if (mode == QuickTileMode(QuickTileFlag::None)) {
        ...
        Tile *tile = TileManager::instance(output(), VirtualDesktopManager::self()->currentDesktop(), workspace()->activities()->current())->bestTileForPosition(keyboard ? moveResizeGeometry().center() : Cursors::self()->mouse()->pos());
        setTile(tile);
    } else {
        Tile *tile = TileManager::instance(output(), VirtualDesktopManager::self()->currentDesktop(), workspace()->activities()->current())->quickTile(mode);
        setTile(tile);
    }
