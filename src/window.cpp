 ...
 * are visible. Visible means inside @p availabeArea and not obstructed by @p restrictedArea.
 */
static std::optional<QPointF> closestUnobstructedGeom(
    const QRectF windowGeom, const QRectF availableArea, const StrutRects restrictedArea,
    const qreal titlebarHeight, const Gravity gravity = Gravity::None, int requiredPixels = 100)
{
    if (gravity == Gravity::Bottom) {
        // Titlebar doesn't move when resizing from the bottom
        return QPointF(0, 0);
    }

    requiredPixels = std::min(std::floor(windowGeom.width()), qreal(requiredPixels));
    // we create a QRegion containing all posible positions for *top-left*
    // of the part of the titlebar that is required to be visible
    QRegion availableRegion(availableArea.x(), availableArea.y(),
                            availableArea.width() - requiredPixels, availableArea.height() - titlebarHeight);
    for (QRect rect : restrictedArea) {
        rect.setTop(rect.top() - titlebarHeight);
        rect.setLeft(rect.left() - requiredPixels);
        availableRegion -= rect;
    }

    const auto closestPointInRect = [](const QRect &rect, const QPointF &point) {
        QPointF closest = point;
        closest.setX(std::clamp(point.x(), qreal(rect.x()), qreal(rect.x() + rect.width())));
        closest.setY(std::clamp(point.y(), qreal(rect.y()), qreal(rect.y() + rect.height())));
        return closest;
    };
    ...
    const bool topLeftMoves = !(gravity == Gravity::Right || gravity == Gravity::TopRight || gravity == Gravity::BottomRight);
    const QPointF anchor = topLeftMoves ? windowGeom.topLeft() : windowGeom.topLeft() + QPointF(windowGeom.width(), 0);

    qreal minDistance = std::numeric_limits<qreal>::max();
    QPointF closestPoint;
    for (QRect rect : availableRegion) {
        // Convert top-left positions of the visible titlebar subrect to top-left positions of anchor.
        // Explicitly consider restrictions imposed by gravity: e.g., when resizing from the right,
        // the right of the window must stay to the right of the left of the window.

        // If rect becomes invalid at any stage of processing, we should continue
        // Otherwise, further processing may erroneously convert rect back to a valid QRect
        if (gravity != Gravity::None) {
            // for resize, must be above windowGeom.bottom()
            const qreal newBottom = windowGeom.bottom() - titlebarHeight;
        ...
        case Gravity::Top:
            ...
            // In this case the width of the window doesn't change
            rect.setLeft(rect.left() - windowGeom.width() + requiredPixels);
            break;
        case (Gravity::Left):
        case (Gravity::TopLeft):
        case (Gravity::BottomLeft): {
            // for these resize gravities, must be to the left of windowGeom.right()
            const qreal newRight = windowGeom.right() - requiredPixels;
        ...
        case (Gravity::TopRight):
        case (Gravity::BottomRight): {
            // for these resize gravities, must be to the right of windowGeom.left()
            const qreal newLeft = windowGeom.left();
            ...
            // Note that in this case top-left of window doesn't move, but top-right does
            // convert top-left of visible titlebar subrect to *top-right* of the window
        ...
        const QPointF dest = closestPointInRect(rect, anchor);
        const qreal distance = calcDistance(anchor, dest);
...
QRectF Window::nextInteractiveResizeGeometry(const QPointF &global) const
{
    ...
    if (!isUnrestrictedInteractiveMoveResize()) {
        const std::optional<QPointF> offset = closestUnobstructedGeom(
            ...
            workspace()->restrictedMoveArea(VirtualDesktopManager::self()->currentDesktop()), titlebarThickness(), gravity);
        if (offset) {
...
QRectF Window::nextInteractiveMoveGeometry(const QPointF &global) const
{
    ...
    if (!isUnrestrictedInteractiveMoveResize()) {
        const std::optional<QPointF> offset = closestUnobstructedGeom(
            nextMoveResizeGeom, workspace()->clientArea(FullArea, this, workspace()->activeOutput()),
            workspace()->restrictedMoveArea(VirtualDesktopManager::self()->currentDesktop()), titlebarThickness());
        if (offset) {
