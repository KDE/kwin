...
private:
    bool genericReply(xcb_atom_t target_P, xcb_atom_t property_P, xcb_window_t requestor_P) override
    {
        if (target_P == xa_version) {
            int32_t version[] = {2, 0};
            xcb_change_property(connection(), XCB_PROP_MODE_REPLACE, requestor_P, property_P, XCB_ATOM_INTEGER, 32, 2, version);
        } else
    ...
    void replyTargets(xcb_atom_t property_P, xcb_window_t requestor_P) override
    {
        ...
        // PropModeAppend !
        xcb_change_property(connection(), XCB_PROP_MODE_APPEND, requestor_P, property_P, XCB_ATOM_ATOM, 32, 1, atoms);
    }
    ...
    void getAtoms() override
    {
        ...
        if (xa_version == XCB_ATOM_NONE) {
            ...
            ScopedCPointer<xcb_intern_atom_reply_t> atom(
                xcb_intern_atom_reply(connection(), xcb_intern_atom_unchecked(connection(), false, name.length(), name.constData()), nullptr));
            if (!atom.isNull()) {
    ...
    xcb_atom_t make_selection_atom(int screen_P)
    {
        ...
        ScopedCPointer<xcb_intern_atom_reply_t> atom(
            xcb_intern_atom_reply(connection(), xcb_intern_atom_unchecked(connection(), false, screen.length(), screen.constData()), nullptr));
        if (atom.isNull()) {
...
{
    for (xcb_screen_iterator_t it = xcb_setup_roots_iterator(xcb_get_setup(connection)); it.rem; --screen, xcb_screen_next(&it)) {
        if (screen == 0) {
            return it.data;
        }
    }
...
void ApplicationX11::performStartup()
{
    ...
    connect(owner.data(), &KSelectionOwner::claimedOwnership, this, [this] {
        ...
        ScopedCPointer<xcb_generic_error_t> redirectCheck(
            xcb_request_check(connection(), xcb_change_window_attributes_checked(connection(), rootWindow(), XCB_CW_EVENT_MASK, maskValues)));
...
void ApplicationX11::crashHandler(int signal)
{
    ...
    char cmd[1024];
    sprintf(cmd, "%s --crashes %d &", QFile::encodeName(QCoreApplication::applicationFilePath()).constData(), crashes);
...
int main(int argc, char *argv[])
{
    ...
    if (number_of_screens != 1 && isMultiHead()) {
        ...
        const QString envir = QStringLiteral("DISPLAY=%1.%2").arg(display_name.data()).arg(KWin::Application::x11ScreenNumber());
