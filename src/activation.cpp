...
bool Workspace::mayActivate(Window *window, const QString &token) const
{
    ...
    if (!m_activationToken.isEmpty() && token == m_activationToken && input()->lastFocusInputSerial() <= m_activationTokenSerial) {
        return true;
    ...
    if (!m_activationToken.isEmpty()
        && input()->lastFocusInputSerial() <= m_activationTokenSerial
