...
XdgSurfaceConfigure *XdgToplevelWindow::sendRoleConfigure() const
{
    ...
    if (nextClientSize.isEmpty()) {
        ...
        m_shellSurface->sendConfigureBounds(bounds.toSize()); // DAVE rounding
    }
...
XdgSurfaceConfigure *XdgPopupWindow::sendRoleConfigure() const
{
    ...
    const quint32 serial = m_shellSurface->sendConfigure(QRect(popupPosition.toPoint(), moveResizeGeometry().size().toSize())); // DAVE rounding
