...
void WaylandCompositor::composite(RenderLoop *renderLoop)
{
    ...
    if (!output->currentBrightness().has_value()
        ...
        frame->setBrightness(output->brightnessSetting() * output->dimmingMultiplier());
    } else {
        ...
        frame->setBrightness(std::pow(std::clamp(std::pow(output->brightnessSetting() * output->dimmingMultiplier(), 1.0 / 2.2), current - maxChangePerFrame, current + maxChangePerFrame), 2.2));
    }
