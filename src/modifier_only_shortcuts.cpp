...
void ModifierOnlyShortcuts::keyEvent(KeyEvent *event)
{
    ...
    if (event->type() == QEvent::KeyPress) {
        ...
        if (wasEmpty && m_pressedKeys.size() == 1 && !ScreenLockerWatcher::self()->isLocked() && m_pressedButtons == Qt::NoButton
        ...
        if (m_pressedKeys.isEmpty() && event->modifiersRelevantForGlobalShortcuts() == Qt::NoModifier && workspace()
