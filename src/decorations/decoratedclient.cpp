    ...
    , m_active(window->isActive())
{
    ...
    connect(window, &Window::activeChanged, this, [this, decoratedClient, window]() {
        if (!window->isZombie()) {
            m_active = window->isActive();
            Q_EMIT decoratedClient->activeChanged(m_active);
        }
    });
...
{
    if (!DecorationBridge::self()->showToolTips() || m_client->isDeleted()) {
        return;
    }
