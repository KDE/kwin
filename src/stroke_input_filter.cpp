...
void StrokeInputFilter::beginStroke(ButtonGrab &grab, PointerMotionEvent *event)
{
    ...
    KWin::input()->pointer()->processStrokeGestureBegin(grab.modifiers, grab.points.first(), grab.points.last(), event->timestamp, m_activeGrabDevice);
    ...
    if (currentDistanceFromOrigin > m_simplificationRadialDistanceTolerance) {
        if (m_simplificationToleranceVector == QPointF()) {
            ...
            if (startingNewSegment) {
                ...
                KWin::input()->pointer()->processStrokeGestureUpdate(grab.points.last(), StrokeGestureUpdate::UpdatingCurrentSegment, event->timestamp, m_activeGrabDevice);

                // Then continue the line to current pointer position as end of the new line segment.
                grab.points.push_back(event->position);
                KWin::input()->pointer()->processStrokeGestureUpdate(grab.points.last(), StrokeGestureUpdate::StartingNewSegment, event->timestamp, m_activeGrabDevice);
