...
void StrokeInputFilter::reconfigure(const KConfigGroup &config)
{
    ...
    m_defaultActivationButton = static_cast<Qt::MouseButton>(config.readEntry("DefaultActivationButton", qToUnderlying(Qt::MouseButton::NoButton)));
    if (m_defaultActivationButton == Qt::MouseButton::LeftButton) { // sorry, denied
        m_defaultActivationButton = Qt::MouseButton::NoButton;
    }
...
bool StrokeInputFilter::pointerButtonPressed(PointerButtonEvent *event)
{
    ...
    Qt::MouseButton activationButton = m_activationButton.value(event->device->name(), m_defaultActivationButton);
