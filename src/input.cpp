...
public:
    ...
    bool tabletToolProximityEvent(TabletToolProximityEvent *event) override
    {
        ...
        // Sync with mouse cursor position once the tablet tool leaves proximity
        if (event->type == TabletToolProximityEvent::LeaveProximity && input()->syncTabletWithMouse()) {
            input()->pointer()->processMotionAbsolute(event->position, event->timestamp, event->device);
        }
