...
bool Item::scheduleFrame(std::optional<std::chrono::steady_clock::time_point> targetTime)
{
    if (!isVisible()) {
        return false;
    }
...
bool Item::scheduleFrameRecursive(std::optional<std::chrono::steady_clock::time_point> targetTime)
{
    if (!isVisible() || !m_scene) {
        return false;
    }
    bool ret = scheduleFrame(targetTime);
