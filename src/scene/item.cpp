...
QRect Item::paintedArea(MainSceneView *delegate, const QRectF &rect) const
{
...
QRegion Item::paintedArea(MainSceneView *delegate, const QRegion &region) const
{
    if (region.isEmpty()) {
        return QRegion();
    }
...
void Item::scheduleRepaint(MainSceneView *delegate, const QRegion &region)
{
    if (isVisible()) {
...
void Item::scheduleRepaintInternal(const QRegion &region)
{
    ...
    const QList<MainSceneView *> delegates = m_scene->delegates();
...
void Item::scheduleRepaintInternal(MainSceneView *delegate, const QRegion &region)
{
    if (Q_UNLIKELY(!m_scene)) {
        return;
    }
