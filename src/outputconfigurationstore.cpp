...
std::pair<OutputConfiguration, QList<Output *>> OutputConfigurationStore::generateConfig(const QList<Output *> &outputs, bool isLidClosed)
{
    ...
    for (const auto output : outputs) {
        ...
        *changeset = {
            ...
            .colorPowerTradeoff = existingData.colorPowerTradeoff.value_or(Output::ColorPowerTradeoff::Balanced),
        };
        if (enable) {
...
void OutputConfigurationStore::load()
{
    ...
    for (const auto &output : outputs) {
        ...
        if (const auto it = data.find("colorPowerTradeoff"); it != data.end()) {
            const auto str = it->toString();
            if (str == "PreferLowPower") {
                state.colorPowerTradeoff = Output::ColorPowerTradeoff::PreferLowPower;
            } else if (str == "Balanced") {
                state.colorPowerTradeoff = Output::ColorPowerTradeoff::Balanced;
    ...
    for (const auto &output : m_outputs) {
        ...
        if (output.colorPowerTradeoff) {
            ...
            case Output::ColorPowerTradeoff::PreferLowPower:
                o["colorPowerTradeoff"] = "PreferLowPower";
                break;
            case Output::ColorPowerTradeoff::Balanced:
                o["colorPowerTradeoff"] = "Balanced";
                break;
