...
QString OutputConfigurationStore::generateId(Output *output) const
{
    QString base;
    if (output->edid().isValid()) {
        base = output->edid().pnpId();
    } else {
        base = QStringLiteral("display");
    }
    if (!output->physicalSize().isEmpty()) {
        const int sizeInches = std::round(std::hypot<double>(output->physicalSize().width(), output->physicalSize().height())) / 25.4;
        base += "-" + QString::number(sizeInches);
    }
    const auto isUnique = [this](QString id) {
        return std::ranges::none_of(m_outputs, [&id](const auto &data) {
            return data.uuid == id;
        });
    };
    if (isUnique(base)) {
    ...
    int number = 2;
    while (!isUnique(base + QString::number(number))) {
        number++;
    }
