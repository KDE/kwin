...
void Placement::placeSmart(Window *window, const QRectF &area, PlacementPolicy /*next*/)
{
    ...
    for (auto k = workspace()->stackingOrder().constEnd() - 1; k >= workspace()->stackingOrder().constBegin(); --k) {
        ...
        int x = client->frameGeometry().right();
        if (x >= area.left() && x + window->width() <= area.right() && !xPossible.contains(x)) {
            xPossible.append(x);
        }
        int y = client->frameGeometry().bottom();
        if (y >= area.top() && y + window->height() <= area.bottom() && !yPossible.contains(y)) {
            yPossible.append(y);
        }
    }
    ...
    for (auto i = xPossible.constBegin(); i != xPossible.constEnd() && overlapOptimal != 0; ++i) {
        for (auto j = yPossible.constBegin(); j != yPossible.constEnd() && overlapOptimal != 0; ++j) {
            ...
            for (int k = relevantClients.length() - 1; k >= 0; --k) {
                ...
                float weight = (client->keepBelow()) ? (qreal)1 / 16 : client->keepAbove() ? 16
                                                                                           : 1 + pow(2, 3 - k);
                QRectF intersection = QRectF(posCurrent, window->size()).intersected(client->frameGeometry());
                float overlap = static_cast<float>(static_cast<int>(weight * intersection.width() * intersection.height() * 10)) / 10.;
