...
static bool maybeApplyNextInputScaling(DrmAbstractColorOp *hardwareOp, DrmAbstractColorOp *previousHardwareOp, Assignments &assignments)
{
    ...
    if (previousHardwareOp == hardwareOp && !hardwareOp->supportsMultipleOps()) {
        return false;
    }
    if (previousHardwareOp != hardwareOp && hardwareOp->inputScaling(*assignments.nextInverseInputScaling)) {
        return false;
    }
    if (hardwareOp->outputScaling(*assignments.nextInverseInputScaling)) {
        return false;
    }
...
static std::optional<Assignments> findColorPipelineAssignments(DrmAbstractColorOp *hardwareOp, DrmAbstractColorOp *previousHardwareOp, std::span<const ColorOp> ops, const std::optional<ColorOp> &previousInverseOutputScaling)
{
    ...
    if (!preference) {
        ...
        if (ret) {
            maybeApplyNextInputScaling(hardwareOp, previousHardwareOp, *ret);
        }
        return ret;
    }
    if (hardwareOp->canBypass()) {
    ...
    if (!ret) {
        ...
        return findColorPipelineAssignments(nextHardwareOp->next(), previousHardwareOp, ops, previousInverseOutputScaling);
    }
    ...
    if (!maybeApplyNextInputScaling(hardwareOp, hardwareOp, *ret)) {
        return std::nullopt;
    }
