...
DrmPipeline::Error DrmPipeline::prepareAtomicPresentation(DrmAtomicCommit *commit, const std::shared_ptr<OutputFrame> &frame)
{
    ...
    // crtc level properties can only apply the same transformation to all planes
    const bool differentPipelines = std::ranges::any_of(m_pending.layers | std::views::drop(1), [this](DrmPipelineLayer *layer) {
