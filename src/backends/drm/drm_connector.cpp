...
bool DrmConnector::updateProperties()
{
    ...
    if (auto blob = tile.immutableBlob()) {
        ...
        const bool parsed = sscanf(static_cast<char *>(blob->data), "%d:%d:%d:%d:%d:%d:%d:%d",
                                   &groupId, &flags, &numTilesX, &numTilesY, &tileLocX, &tileLocY, &tileWidth, &tileHeight)
            == 0;
        if (parsed && numTilesX > 1 && numTilesY > 1) {
            ...
            qWarning() << "Connector" << connectorName() << "has tile info" << groupId << m_tileInfo->completeSizeInTiles << m_tileInfo->tileLocation << m_tileInfo->tileSize;
        } else {
