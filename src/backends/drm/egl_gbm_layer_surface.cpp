...
std::optional<QRegion> EglGbmLayerSurface::startRendering(const QSize &bufferSize, const QRect &geometry, DrmPlane::Transformations renderTransform,
                                                          DrmPlane::Transformations bufferTransform, const QMap<uint32_t, QVector<uint64_t>> &formats)
{
    ...
    if (doesShadowBufferFit(m_shadowBuffer.data(), renderSize, renderTransform, bufferTransform)) {
        ...
        if (doesShadowBufferFit(m_oldShadowBuffer.data(), renderSize, renderTransform, bufferTransform)) {
            ...
            if (renderTransform != bufferTransform) {
                ...
                m_shadowBuffer = QSharedPointer<ShadowBuffer>::create(renderSize, format);
    ...
    if (m_shadowBuffer) {
        ...
        return m_gbmSurface->repaintRegion(geometry);
    }
}
