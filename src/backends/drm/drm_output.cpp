...
std::shared_ptr<ColorDescription> DrmOutput::createColorDescription(const State &next) const
{
    ...
    if (next.colorProfileSource == ColorProfileSource::ICC && !effectiveHdr && !effectiveWcg && next.iccProfile) {
        ...
        return std::make_shared<ColorDescription>(ColorDescription{
            ...
            maxFALL * next.maxPossibleAritificalHdrHeadroom,
            maxFALL * next.maxPossibleAritificalHdrHeadroom,
            next.iccProfile->colorimetry(),
            sdrColor,
        });
    ...
    const double maxAverageBrightness = effectiveHdr ? next.maxAverageBrightnessOverride.value_or(m_connector->edid()->desiredMaxFrameAverageLuminance().value_or(next.referenceLuminance)) : 200 * next.maxPossibleAritificalHdrHeadroom;
    const double maxPeakBrightness = effectiveHdr ? next.maxPeakBrightnessOverride.value_or(m_connector->edid()->desiredMaxLuminance().value_or(800)) : 200 * next.maxPossibleAritificalHdrHeadroom;
...
bool DrmOutput::queueChanges(const std::shared_ptr<OutputChangeSet> &props)
{
    ...
    m_nextState->maxPossibleAritificalHdrHeadroom = calculateMaxArtificialHdrHeadroom(*m_nextState);
...
void DrmOutput::updateBrightness(double newBrightness, double newArtificialHdrHeadroom, double newDimming)
{
    ...
    next.maxPossibleAritificalHdrHeadroom = calculateMaxArtificialHdrHeadroom(next);
