...
void DrmOutput::applyQueuedChanges(const std::shared_ptr<OutputChangeSet> &props)
{
    ...
    next.colorDescription = createColorDescription(props, m_state.currentBrightness);
...
void DrmOutput::setBrightnessDevice(BrightnessDevice *device)
{
    ...
    updateBrightness(m_state.currentBrightness);
}
...
bool DrmOutput::setChannelFactors(const QVector3D &rgb)
{
    if (rgb != m_channelFactors) {
        ...
        State next = m_state;
        next.colorDescription = createColorDescription(std::make_shared<OutputChangeSet>(), next.currentBrightness);
        setState(next);
        tryKmsColorOffloading();
    }
    return true;
