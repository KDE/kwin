            ...
            const auto createSwapchain = [&formats, this](bool requireAlpha) {
                for (auto it = formats.begin(); it != formats.end(); it++) {
                    ...
                    if (m_surface->shadowSwapchain) {
                        qWarning() << "created swapchain with format" << drmGetFormatName(m_surface->shadowSwapchain->format()) << "and modifier" << drmGetFormatModifierName(m_surface->shadowSwapchain->modifier());
                        break;
                    }
                }
            };
