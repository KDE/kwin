...
bool WaylandEglPrimaryLayer::doEndFrame(const QRegion &renderedRegion, const QRegion &damagedRegion, OutputFrame *frame)
{
    ...
    static_cast<WaylandOutput *>(m_output.get())->setPrimaryBuffer(m_backend->backend()->importBuffer(m_buffer->buffer()), damagedRegion);
...
bool WaylandEglPrimaryLayer::doImportScanoutBuffer(GraphicsBuffer *buffer, const std::shared_ptr<OutputFrame> &frame)
{
    ...
    if (presentationBuffer) {
        static_cast<WaylandOutput *>(m_output.get())->setPrimaryBuffer(presentationBuffer, targetRect());
    }
