...
void FakeInputBackendPrivate::org_kde_kwin_fake_input_keyboard_keysym(Resource *resource, uint32_t keysym, uint32_t state)
{
    ...
    if (result) {
        const auto keycode = result->first;
        const auto level = result->second;
        ...
        if (level == 1) {
            ...
            qCWarning(KWIN_FAKEINPUT) << "Did not handle key level" << level;
        }

        org_kde_kwin_fake_input_keyboard_key(resource, keycode, state);
    } else {
