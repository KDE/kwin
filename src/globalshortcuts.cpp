    ...
    , m_action(action)
{
    if (auto swipeGesture = std::get_if<RealtimeFeedbackSwipeShortcut>(&sc)) {
        ...
        m_strokeGesture = std::make_unique<StrokeGesture>();
        m_strokeGesture->setStroke({strokeGesture->points});
    }
}
...
void GlobalShortcutsManager::registerStroke(const std::vector<QPointF> &points, QAction *action)
{
    if (m_strokeGestures) {
