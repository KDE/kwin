...
void GlobalShortcutsManager::init()
{
    ...
    if (!m_kglobalAccel->init()) {
        ...
        QObject::connect(m_kglobalAccel.get(),
                         &KGlobalAccelD::triggerActive,
                         this,
                         &GlobalShortcutsManager::onKGlobalShortcutTriggerActive,
...
bool GlobalShortcutsManager::processKey(Qt::KeyboardModifiers mods, int keyQt, KeyboardKeyState state)
{
    ...
    case KWin::KeyboardKeyState::Pressed:
        return m_kglobalAccel->keyEvent(int(mods) | keyQt, ShortcutKeyState::Pressed);
    case KWin::KeyboardKeyState::Repeated:
        return m_kglobalAccel->keyEvent(int(mods) | keyQt, ShortcutKeyState::Repeated);
    case KWin::KeyboardKeyState::Released:
        return m_kglobalAccel->keyEvent(int(mods) | keyQt, ShortcutKeyState::Released);
    }
#endif
    return false;
}
...
bool GlobalShortcutsManager::processPointerPressed(Qt::KeyboardModifiers mods, Qt::MouseButtons pointerButtons)
{
    ...
    if (m_kglobalAccel) {
        m_kglobalAccel->pointerPressed(pointerButtons);
    }
#endif
...
bool GlobalShortcutsManager::processAxis(Qt::KeyboardModifiers mods, PointerAxisDirection axis, qreal delta)
{
    ...
    if (m_kglobalAccel) {
        m_kglobalAccel->axisTriggered(axis);
    }
#endif
...
ConfigurableGesture *GlobalShortcutsManager::registerGesture(QAction *shortcutAction)
{
    ...
    for (auto &[trigger, active] : m_kglobalAccelActiveTriggers) {
        if (active.shortcutActionKey == gesture->objectName()) {
            activateGesture(active, gesture, trigger);
        }
    }
    ...
    for (auto &[trigger, active] : m_kglobalAccelActiveTriggers) {
        if (active.configurableGesture == gesture) {
            if (replacementGesture) {
                activateGesture(active, replacementGesture, trigger);
            } else {
                                                            ...
                                                            const QString &actionFriendlyName)
{
    ...
    if (active) {
        ...
        ActiveTriggerInfo &active = m_kglobalAccelActiveTriggers[trigger];
        active.shortcutActionKey = shortcutActionKey;

        activateGesture(active, actionGesture, trigger);
    }

    if (!active) {
        if (auto it = m_kglobalAccelActiveTriggers.find(trigger); it != m_kglobalAccelActiveTriggers.end()) {
            deactivateGesture(it->second);
...
bool GlobalShortcutsManager::activateGesture(ActiveTriggerInfo &active, ConfigurableGesture *gesture, const KGlobalShortcutTrigger &trigger)
{
