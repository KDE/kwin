...
void EffectsHandlerImpl::registerRealtimeTouchpadSwipeShortcut(const QString &context, SwipeDirection dir, uint fingerCount, QAction *onUp, std::function<void(qreal)> progressCallback)
{
    input()->registerRealtimeTouchpadSwipeShortcut(context, dir, fingerCount, onUp, progressCallback);
}

void EffectsHandlerImpl::registerTouchpadSwipeShortcut(const QString &context, SwipeDirection direction, uint fingerCount, QAction *action)
{
    input()->registerTouchpadSwipeShortcut(context, direction, fingerCount, action);
}

void EffectsHandlerImpl::registerRealtimeTouchpadPinchShortcut(const QString &context, PinchDirection dir, uint fingerCount, QAction *onUp, std::function<void(qreal)> progressCallback)
{
    input()->registerRealtimeTouchpadPinchShortcut(context, dir, fingerCount, onUp, progressCallback);
}

void EffectsHandlerImpl::registerTouchpadPinchShortcut(const QString &context, PinchDirection direction, uint fingerCount, QAction *action)
{
    input()->registerTouchpadPinchShortcut(context, direction, fingerCount, action);
}
...
void EffectsHandlerImpl::setGestureContext(const QString &contextName)
{
    input()->setGestureContext(contextName);
}

void EffectsHandlerImpl::resetGestureContext()
{
    input()->resetGestureContext();
}
