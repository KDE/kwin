...
std::shared_ptr<VulkanSwapchainSlot> VulkanSwapchain::acquire()
{
    ...
    if (!buffer) {
        qWarning() << "Failed to allocate a vulkan swapchain buffer";
        return nullptr;
    }
...
void VulkanSwapchain::release(VulkanSwapchainSlot *slot)
{
    // TODO buffer age
}
...
std::shared_ptr<VulkanSwapchain> VulkanSwapchain::create(VulkanDevice *device, GraphicsBufferAllocator *allocator, const QSize &size, uint32_t format, const QVector<uint64_t> &modifiers)
{
    ...
    if (!buffer) {
        qWarning() << "Failed to allocate a graphics buffer for a Vulkan swapchain";
        return nullptr;
    }
