...
static bool isDisjoint(const DmaBufAttributes &attributes)
{
    ...
    if (fstat(attributes.fd[0].get(), &stat1) != 0) {
        qWarning() << "failed to fstat dmabuf";
        return true;
    }
...
std::optional<VulkanTexture> VulkanDevice::importDmabuf(GraphicsBuffer *buffer) const
{
    ...
    std::array queueFamilyIndices{m_queueFamilyIndex};
    ...
    if (imageResult != vk::Result::eSuccess) {
        qWarning() << "creating vulkan image failed!" << vk::to_string(imageResult);
        return std::nullopt;
    }
    ...
    for (int i = 0; i < attributes->planeCount; i++) {
        ...
        if (disjoint) {
            planeInfo[i] = vk::BindImagePlaneMemoryInfo(planeRequirementsInfo.planeAspect);
    ...
    auto [imageViewResult, imageView] = m_logical.createImageViewUnique(vk::ImageViewCreateInfo(
        ...
        vk::ComponentMapping{
            ...
            vk::ComponentSwizzle::eIdentity,
            vk::ComponentSwizzle::eIdentity,
            format->alphaBits > 0 ? vk::ComponentSwizzle::eIdentity : vk::ComponentSwizzle::eOne},
    ...
    if (imageViewResult != vk::Result::eSuccess) {
        qWarning() << "couldn't create image view!" << vk::to_string(imageViewResult);
        return std::nullopt;
    }
...
QHash<uint32_t, QVector<uint64_t>> VulkanDevice::queryFormats(vk::ImageUsageFlags flags) const
{
    ...
    for (const uint32_t drmFormat : s_knownDrmFormats) {
        ...
        vk::FormatProperties2 formatProps({}, &modifierInfos);
        ...
        for (const auto &props : formatModifierProps) {
            vk::PhysicalDeviceImageDrmFormatModifierInfoEXT modifierInfo(props.drmFormatModifier, vk::SharingMode::eExclusive);
            vk::PhysicalDeviceExternalImageFormatInfo externalInfo(vk::ExternalMemoryHandleTypeFlagBits::eDmaBufEXT, &modifierInfo);
            ...
            vk::ImageFormatProperties2 imageProps({}, &externalProps);
            ...
            if (result == vk::Result::eErrorFormatNotSupported) {
                qWarning() << "unsupported format:" << vk::to_string(info.vulkanFormat);
                continue;
            } else if (result != vk::Result::eSuccess) {
                qWarning() << "failed to get image format properties:" << vk::to_string(result);
                continue;
            }
            if (!(externalProps.externalMemoryProperties.externalMemoryFeatures & vk::ExternalMemoryFeatureFlagBits::eImportable)) {
                qWarning() << "can't import format" << vk::to_string(info.vulkanFormat);
                continue;
            }
