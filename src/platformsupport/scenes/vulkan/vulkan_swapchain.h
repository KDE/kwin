...
{
public:
    VulkanSwapchainSlot(GraphicsBuffer *buffer, std::unique_ptr<VulkanTexture> &&texture, vk::UniqueCommandBuffer &&commandBuffer);
    ~VulkanSwapchainSlot();
...
{
public:
    VulkanSwapchain(VulkanDevice *device, GraphicsBufferAllocator *allocator, const QSize &size, uint32_t format, uint64_t modifier, const std::shared_ptr<VulkanSwapchainSlot> &initialSlot);
    ~VulkanSwapchain();
