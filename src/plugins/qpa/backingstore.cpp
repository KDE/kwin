...
void BackingStore::beginPaint(const QRegion &region)
{
    ...
    m_bufferView = std::make_unique<GraphicsBufferView>(m_buffer.buffer(), GraphicsBuffer::Read | GraphicsBuffer::Write);
    ...
    if (oldBuffer && oldBuffer != m_buffer && oldBuffer->size() == m_buffer->size()) {
        const GraphicsBufferView oldView(oldBuffer.buffer(), GraphicsBuffer::Read);
