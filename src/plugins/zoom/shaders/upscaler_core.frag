...
#define PIXEL(x,y) texture(sampler, texcoord0 + pixelSize * vec2(x, y)).rgb
    ...
    //  xx  E1  E2  E3  xx
    //
    // Whether we need to sample the outer cells depends on
    // the pattern in the inner cells. Our "current" pixel is
    // cell C2.
    ...
    // Edge detection
    // We need to look at the 4 corners of the
    // above neighbourhood and decide whether a given corner
    // has a strong diagonal line going through it. We also need
    // to know what direction said line is travelling in.
    //
    // When checking a corner, we sample a 4x4 circular region of
    // pixels and use their color distances to estimate the likelihood
    // of there being a diagonal going through that region.
    //
    // These values are used to determine how we blend pixels in
    // the upscaled image.
    //
    // Blend types are mapped like this:
    //  x=TL y=TR
    //  z=BL w=BR
    ...
    if (!((B1 == B2) && (C1 == C2) || (B1 == C1 && B2 == C2))) {
        ...
        vec3 B0 = PIXEL(-2, -1);
        vec3 C0 = PIXEL(-2, 0);
        float distB1throughC2 = (4 * colorDistance(B1, C2)) + colorDistance(B2, C3) + colorDistance(C1, D2) + colorDistance(A1, B2) + colorDistance(B0, C1);
        float distC1throughB2 = (4 * colorDistance(C1, B2)) + colorDistance(C2, B3) + colorDistance(D1, C2) + colorDistance(B1, A2) + colorDistance(C0, B1);

        bool downwardIsDominant = (DOMINANT_DIRECTION_THRESHOLD * distC1throughB2) < distB1throughC2;

        if (C2 != B2 && C2 != C1 && distC1throughB2 < distB1throughC2) {
    ...
    if (!((B2 == B3) && (C2 == C3) || (B2 == C2 && B3 == C3))) {
        ...
        vec3 B4 = PIXEL(2, -1);
        vec3 C4 = PIXEL(2, 0);
        float distB2throughC3 = (4 * colorDistance(B2, C3)) + colorDistance(B3, C4) + colorDistance(C2, D3) + colorDistance(A2, B3) + colorDistance(B1, C2);
        float distC2throughB3 = (4 * colorDistance(C2, B3)) + colorDistance(C3, B4) + colorDistance(D2, C3) + colorDistance(B2, A3) + colorDistance(C1, B2);

        bool downwardIsDominant = (DOMINANT_DIRECTION_THRESHOLD * distB2throughC3) < distC2throughB3;

        if (C2 != B2 && C2 != C3 && distC2throughB3 > distB2throughC3) {
    ...
    if (!((C1 == C2) && (D1 == D2) || (C1 == D1 && C2 == D2))) {
        ...
        vec3 C0 = PIXEL(-2, 0);
        vec3 D0 = PIXEL(-2, 1);
        float distC1throughD2 = (4 * colorDistance(D2, C1)) + colorDistance(C2, D3) + colorDistance(D1, E2) + colorDistance(B1, C2) + colorDistance(C0, D1);
        float distD1throughC2 = (4 * colorDistance(D1, C2)) + colorDistance(D2, C3) + colorDistance(E1, D2) + colorDistance(C1, B2) + colorDistance(D0, C1);

        bool downwardIsDominant = (DOMINANT_DIRECTION_THRESHOLD * distC1throughD2) < distD1throughC2;

        if (C2 != C1 && C2 != D2 && distD1throughC2 > distC1throughD2) {
    ...
    if (!((C2 == C3) && (D2 == D3) || (C2 == D2 && C3 == D3))) {
        ...
        vec3 C4 = PIXEL(2, 0);
        vec3 D4 = PIXEL(2, 1);
        float distC2throughD3 = (4 * colorDistance(C2, D3)) + colorDistance(C3, D4) + colorDistance(D2, E3) + colorDistance(B2, C3) + colorDistance(C1, D2);
        float distD2throughC3 = (4 * colorDistance(D2, C3)) + colorDistance(D3, C4) + colorDistance(E2, D3) + colorDistance(C2, B3) + colorDistance(D1, C2);

        bool downwardIsDominant = (DOMINANT_DIRECTION_THRESHOLD * distD2throughC3) < distC2throughD3;

        if (C2 != C3 && C2 != D2 && distD2throughC3 < distC2throughD3) {
            blendTypes.w = downwardIsDominant ? BLEND_DOMINANT : BLEND_NORMAL;
        } else {
    ...
    // Blending stage
    //
    // Based on the blending types decided above, we now
    // fill in each corner of the destination pixel by deciding
    // whether we should:
    //
    // - not blend at all, just fill with source color
    // - blend as a corner between two pixels
    // - split the region in half and fill each side with different colors
    //
    // We split the region if the region is marked as dominant or if the source
    // region's shape doesn't look too flat or ambiguous.
    //
    // We always split the region in half by a line, but we need to decide
    // whether the line we use comes from the image or if we should just split
    // the pixel diagonally down the middle.
    vec3 color = C2;

    // color += blendTypeToDebugColor(blendTypes.x); // * step(pixelOffset.x, 0) * step(pixelOffset.y, 0);
    // color += blendTypeToDebugColor(blendTypes.y) * step(0, pixelOffset.x) * step(pixelOffset.y, 0);
    // color += blendTypeToDebugColor(blendTypes.z) * step(pixelOffset.x, 0) * step(0, pixelOffset.y);
    // color += blendTypeToDebugColor(blendTypes.w) * step(0, pixelOffset.x) * step(0, pixelOffset.y);
