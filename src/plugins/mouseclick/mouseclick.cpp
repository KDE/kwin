...
bool MouseClickEffect::paintScreen(const RenderTarget &renderTarget, const RenderViewport &viewport, int mask, const Region &deviceRegion, LogicalOutput *screen)
{
    ...
    loadShaders();
    if (!m_shader || !m_simulationShader) {
        return effects->paintScreen(renderTarget, viewport, mask, deviceRegion, screen);
    }
    ...
    if (!capture.scene.texture || capture.scene.texture->size() != nativeSize || capture.scene.texture->internalFormat() != textureFormat) {
        ...
        for (OffscreenTarget &simulation : capture.simulation) {
            simulation.texture = GLTexture::allocate(textureFormat, nativeSize);
            if (!simulation.texture) {
                m_screenCaptures.remove(screen);
                return effects->paintScreen(renderTarget, viewport, mask, deviceRegion, screen);
            }
            simulation.framebuffer = std::make_shared<GLFramebuffer>(simulation.texture.get());
            if (!simulation.framebuffer->valid()) {
                m_screenCaptures.remove(screen);
                return effects->paintScreen(renderTarget, viewport, mask, deviceRegion, screen);
            }

            GLFramebuffer::pushFramebuffer(simulation.framebuffer.get());
            glClearColor(0, 0, 0, 0);
            glClear(GL_COLOR_BUFFER_BIT);
            GLFramebuffer::popFramebuffer();
        }
    ...
    const OffscreenTarget &previousFrame = capture.simulation[capture.previousFrame];
    const OffscreenTarget &nextSimulation = capture.simulation[nextFrame];
    GLFramebuffer::pushFramebuffer(nextSimulation.framebuffer.get());
    glActiveTexture(GL_TEXTURE1);
    previousFrame.texture->bind();
    glActiveTexture(GL_TEXTURE0);
    ShaderManager::instance()->pushShader(m_simulationShader.get());
    QMatrix4x4 simulationProjectionMatrix;
    simulationProjectionMatrix.ortho(QRectF(QPointF(), nativeSize));
    m_simulationShader->setUniform(m_simulationModelViewProjectionMatrixLocation, simulationProjectionMatrix);
    m_simulationShader->setUniform("sampler", 0);
    m_simulationShader->setUniform(m_simulationTimeLocation, int(m_click ? m_click->m_time : 0));
    m_simulationShader->setUniform(m_simulationPreviousFrameLocation, 1);
    m_simulationShader->setUniform(m_simulationCursorPositionLocation, cursorPosition);
    m_simulationShader->setUniform(m_cursorRadiusLocation, cursorRadius);
    m_simulationShader->setUniform(m_simulationScreenSizeLocation, QVector2D(nativeSize.width(), nativeSize.height()));
    ...
    glActiveTexture(GL_TEXTURE0);
    const OffscreenTarget &simulationFrame = capture.simulation[capture.previousFrame];
    glActiveTexture(GL_TEXTURE1);
    simulationFrame.texture->bind();
    glActiveTexture(GL_TEXTURE0);
    ...
    m_shader->setUniform("simulationTexture", 1);
    ...
    ShaderManager::instance()->popShader();
    glActiveTexture(GL_TEXTURE1);
    simulationFrame.texture->unbind();
    glActiveTexture(GL_TEXTURE0);
...
void MouseClickEffect::loadShaders()
{
    ...
    if (!m_simulationShader) {
        m_simulationShader = ShaderManager::instance()->generateShaderFromFile(ShaderTrait::MapTexture,
                                                                               QStringLiteral(":/effects/mouseclick/shaders/mouseclick.vert"),
                                                                               QStringLiteral(":/effects/mouseclick/shaders/simulation.frag"));
        if (!m_simulationShader) {
            qWarning("Failed to load the mouseclick simulation shader");
        } else {
            m_simulationModelViewProjectionMatrixLocation = m_simulationShader->uniformLocation("modelViewProjectionMatrix");
            m_simulationTimeLocation = m_simulationShader->uniformLocation("time");
            m_simulationCursorPositionLocation = m_simulationShader->uniformLocation("cursorPosition");
            m_cursorRadiusLocation = m_simulationShader->uniformLocation("cursorRadius");
            m_simulationPreviousFrameLocation = m_simulationShader->uniformLocation("previousFrame");
            m_simulationScreenSizeLocation = m_simulationShader->uniformLocation("screenSize");
        }
    }
}
