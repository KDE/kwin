...
bool ButtonRebindsFilter::tabletPadStripEvent(KWin::TabletPadStripEvent *event)
{
    ...
    qInfo() << event->time << "tabletPadStripEvent" << "callFunc" << event->number << event->position;
    ...
    if (event->position != -1) {
        ...
        qInfo() << event->time << "tabletPadStripEvent" << "m_initialStripPosition =" << m_initialStripPosition;
        ...
        // If accepted (that means the threshold is met) then we want to reset ourselves
        if (sent) {
            qInfo() << event->time << "tabletPadStripEvent" << "event sent!";
            m_initialStripPosition = event->position;
        }
        return sent;
    } else {
