    ...
    , m_inhibitorWatcher(new QDBusServiceWatcher(this))
{
    ...
    QDBusConnection::sessionBus().registerService(QStringLiteral("org.kde.NightLight"));
}

NightLightDBusInterface::~NightLightDBusInterface()
{
    QDBusConnection::sessionBus().unregisterService(QStringLiteral("org.kde.NightLight"));
}
...
{
    return true; // TODO: Night Light should register its own dbus service instead.
}
...
void NightLightDBusInterface::nightLightAutoLocationUpdate(double latitude, double longitude)
{
