...
public:
    ...
    void drawWindow(const RenderTarget &renderTarget, const RenderViewport &viewport, EffectWindow *w, int mask, const RegionF &region, WindowPaintData &data) override;
...
private:
    ...
    QRectF expand(const QRectF &rect) const;
    RegionF expand(const RegionF &region) const;
    void initBlurStrengthValues();
    bool updateTexture(EffectScreen *screen, const RenderTarget &renderTarget);
    RegionF blurRegion(const EffectWindow *w) const;
    QRegion decorationBlurRegion(const EffectWindow *w) const;
    bool decorationSupportsBlurBehind(const EffectWindow *w) const;
    bool shouldBlur(const EffectWindow *w, int mask, const WindowPaintData &data) const;
    void updateBlurRegion(EffectWindow *w);
    void doBlur(const RenderTarget &renderTarget, const RenderViewport &viewport, const RegionF &shape, const QRectF &screen, const float opacity, bool isDock, QRectF windowRect);
    void uploadRegion(QVector2D *&map, const RegionF &region);
    Q_REQUIRED_RESULT bool uploadGeometry(GLVertexBuffer *vbo, const RegionF &expandedBlurRegion, const RegionF &blurRegion);
    ...
    void upscaleRenderToScreen(const ScreenData &data, const RenderTarget &renderTarget, const RenderViewport &viewport, GLVertexBuffer *vbo, int vboStart, int blurRectCount);
    void applyNoise(const ScreenData &data, const RenderTarget &renderTarget, const RenderViewport &viewport, GLVertexBuffer *vbo, int vboStart, int blurRectCount, QPointF windowPosition);
...
private:
    ...
    RegionF m_paintedArea; // keeps track of all painted areas (from bottom to top)
    RegionF m_currentBlur; // keeps track of the currently blured area of the windows(from bottom to top)
    ...
    QMap<const EffectWindow *, RegionF> blurRegions;
