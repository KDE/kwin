...
QString MouseKeysInputDevice::sysName() const
{
    return {};
}
...
void MouseKeysFilter::handleKeyEvent(KWin::KeyEvent *event)
{
    ...
    if (!m_keyStates[event->nativeScanCode()] && keyDown && m_currentKey == 0) {
    ...
    } else if (m_keyStates[event->nativeScanCode()] && !keyDown && event->nativeScanCode() == m_currentKey) {
...
bool MouseKeysFilter::keyEvent(KWin::KeyEvent *event)
{
    ...
    if (event->nativeScanCode() == KEY_KP5) {
        ...
        if (event->type() == QEvent::KeyPress) {
            Q_EMIT m_inputDevice->pointerButtonChanged(m_mouseButton, KWin::InputRedirection::PointerButtonPressed, time, m_inputDevice.get());
        } else {
            Q_EMIT m_inputDevice->pointerButtonChanged(m_mouseButton, KWin::InputRedirection::PointerButtonReleased, time, m_inputDevice.get());
        }
        return true;
    }
