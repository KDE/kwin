...
{
    m_sceneView->setViewportOverride(m_region, m_scale);
...
{
    m_layer->setFramebuffer(target, scaleRegion(bufferDamage, 1.0 / devicePixelRatio(), QRect(QPoint(), m_sceneView->viewport().size().toSize())));
...
void RegionScreenCastSource::resume()
{
    ...
    m_active = true;
    connect(m_layer.get(), &OutputLayer::repaintScheduled, this, &RegionScreenCastSource::report, Qt::QueuedConnection);
}
