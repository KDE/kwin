...
void ShowFpsEffect::prePaintScreen(ScreenPrePaintData &data, std::chrono::milliseconds presentTime)
{
    ...
    if (!m_scene) {
        ...
        const auto url = QUrl(QStringLiteral("qrc:/org.kde.kwin/showfps/qml/main.qml"));
        ...
        if (!m_scene->rootItem()) {
            ...
            const auto fallbackUrl = QUrl(QStringLiteral("qrc:/org.kde.kwin/showfps/qml/main-fallback.qml"));
