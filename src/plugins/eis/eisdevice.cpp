...
void EisDevice::sendKeySym(xkb_keysym_t keySym, KeyboardKeyState state)
{
    ...
    if (keyCode) {
        ...
        if (isPress) {
            ...
            pressedKeys.remove(keyCode->keyCode);
    ...
    if (isPress) {
        ...
        pressedKeys.remove(unmappedKeyCode);
        Q_EMIT keyChanged(unmappedKeyCode, KeyboardKeyState::Released, currentTime(), this);
    }

    // reset keymap back, ideally this should be done a bit more lazily next time we get another event
