...
class XXWmPipV1InterfacePrivate;

enum class Gravity;

/**
 * The XXWmPipV1Interface extension provides clients a way to create picture-in-picture
 * surfaces.
 */
class KWIN_EXPORT XXWmPipV1Interface : public QObject
{
    Q_OBJECT

public:
    explicit XXWmPipV1Interface(Display *display, QObject *parent = nullptr);
    ~XXWmPipV1Interface() override;
...
private:
    std::unique_ptr<XXWmPipV1InterfacePrivate> d;
};
...
public:
    XXPipV1Interface(XXWmPipV1Interface *shell, XdgSurfaceInterface *xdgSurface, wl_resource *resource);
