...
{
public:
    TabletPadGroupV2InterfacePrivate(quint32 modeCount, quint32 currentMode, TabletPadGroupV2Interface *q)
...
TabletPadGroupV2Interface::TabletPadGroupV2Interface(quint32 modeCount, quint32 currentMode, TabletPadV2Interface *parent)
    : QObject(parent)
    , d(new TabletPadGroupV2InterfacePrivate(modeCount, currentMode, this))
{
    d->m_pad = parent;
}
...
public:
        ...
        , m_display(display)
    {
        ...
        for (const InputDeviceTabletPadModeGroup &group : std::as_const(groups)) {
            m_groups += new TabletPadGroupV2Interface(group.modeCount, group.currentMode, q);
        }
    }
...
TabletPadV2Interface::TabletPadV2Interface(const QString &path,
    ...
    : QObject(parent)
    , d(new TabletPadV2InterfacePrivate(path, buttons, rings, strips, groups, display, this))
{
    d->m_seat = parent;
}
...
void TabletPadV2Interface::setCurrentSurface(SurfaceInterface *surface, TabletV2Interface *tablet)
{
    ...
    if (surface) {
        ...
        for (TabletPadGroupV2Interface *group : std::as_const(d->m_groups)) {
            group->sendModeSwitch(0, d->m_display->nextSerial(), group->d->m_currentMode);
        }
    }
}
...
{
    auto iface = new TabletPadV2Interface(device->sysPath(), device->tabletPadButtonCount(), device->tabletPadRingCount(), device->tabletPadStripCount(), device->modeGroups(), d->m_display, this);
    iface->d->m_seat = this;
