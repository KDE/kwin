...
class PresentationTimeFeedback : public QtWaylandServer::wp_presentation_feedback
{
public:
    explicit PresentationTimeFeedback(SurfaceInterface *surface, wl_client *client, uint32_t id);
    ~PresentationTimeFeedback() override;

    void presented(uint64_t msc, std::chrono::nanoseconds refreshCycleDuration, std::chrono::nanoseconds timestamp, PresentationMode mode);
