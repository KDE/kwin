...
QList<TouchInterfacePrivate::Resource *> TouchInterfacePrivate::touchesForClient(wl_client *client) const
{
    return resourceMap().values(client);
}
...
void TouchInterface::sendMotion(SurfaceInterface *surface, qint32 id, const QPointF &localPos)
{
    ...
    for (TouchInterfacePrivate::Resource *resource : touchResources) {
        ...
        d->m_clientsInFrame.insert(resource->client());
    }
}
