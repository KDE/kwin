...
private:
    ...
    void wp_color_representation_manager_v1_create(Resource *resource, uint32_t color_representation, struct ::wl_resource *surface) override;
};

class ColorRepresentationV1 : private QtWaylandServer::wp_color_representation_v1
{
public:
    explicit ColorRepresentationV1(SurfaceInterface *surface, wl_client *client, uint32_t id, uint32_t version);
    ~ColorRepresentationV1();
...
private:
    ...
    void wp_color_representation_v1_set_coefficients_and_range(Resource *resource, uint32_t coefficients, uint32_t range) override;
    void wp_color_representation_v1_set_chroma_location(Resource *resource, uint32_t chroma_location) override;
