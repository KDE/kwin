                                                                               ...
                                                                               uint32_t max_cll, uint32_t max_fall)
{
    ...
    if (max_fall != 0 && (!m_maxPeakBrightness || max_fall <= *m_maxPeakBrightness)) {
        ...
        if (!m_maxPeakBrightness) {
            m_maxPeakBrightness = max_fall;
        }
    }
    const double minLuminance = min_display_mastering_luminance / 10'000.0;
    if ((!m_maxPeakBrightness && !m_maxPeakBrightness)
...
void FrogColorManagementSurfaceV1::updateColorDescription()
{
    if (m_surface) {
        ...
        priv->pending->colorDescription = ColorDescription(m_containerColorimetry, m_transferFunction, priv->preferredColorDescription.value_or(ColorDescription::sRGB).referenceLuminance(), m_minMasteringLuminance.value_or(0), m_maxAverageLuminance, m_maxPeakBrightness, m_masteringColorimetry, Colorimetry::fromName(NamedColorimetry::BT709));
        priv->pending->colorDescriptionIsSet = true;
    }
}
