...
public:
    ...
    /**
     * Notify the real phyiscal key press or release state.
     *
     * This is used to maintain the pressed key state for keyboard enter signal.
     * This does not send any key event to the client, but only affects the key
     * being sent to the client when the keyboard focus changes.
     */
    void updateKeyState(quint32 keyCode, KeyboardKeyState state);

    /**
     * Forward a keyboard key event to client if there's a focused keyboard surface.
     *
     * The key can be from originated from input method, or from the real
     * physical key event, or just synethetic key event generated by kwin itself.
     */
