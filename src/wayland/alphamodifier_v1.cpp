...
void AlphaModifierManagerV1::wp_alpha_modifier_v1_get_surface(Resource *resource, uint32_t id, ::wl_resource *surface)
{
    ...
    new AlphaModifierSurfaceV1(resource->client(), id, std::min<uint32_t>(s_version, resource->version()), surf);
}
...
{
    if (!m_surface) {
        wl_resource_post_error(resource->handle, error_no_surface, "wl_surface was destroyed before the alpha modifier surface");
        return;
    }
    wl_resource_destroy(resource->handle);
}
