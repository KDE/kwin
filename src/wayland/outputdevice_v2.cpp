...
public:
    ...
    uint32_t m_automaticMaxBitsPerColor = 0;
...
void OutputDeviceV2InterfacePrivate::sendMaxBpc(Resource *resource)
{
    if (resource->version() >= KDE_OUTPUT_DEVICE_V2_MAX_BITS_PER_COLOR_SINCE_VERSION) {
        send_max_bits_per_color(resource->handle, m_maxBpc.value_or(0), m_automaticMaxBitsPerColor, m_maxBpcRange.min, m_maxBpcRange.max);
    }
}
...
void OutputDeviceV2Interface::updateMaxBpc()
{
    if (d->m_maxBpc != d->m_handle->maxBitsPerColor()
        ...
        || d->m_automaticMaxBitsPerColor != d->m_handle->automaticMaxBitsPerColor()) {
