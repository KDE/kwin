    ...
    , surface(surface)
{
    ...
    if (surfacePrivate->preferredScale != 1.0) {
        send_preferred_scale(wl_fixed_from_double(surfacePrivate->preferredScale));
    }
}
...
void FractionalScaleV1Interface::wp_fractional_scale_v1_destroy_resource(Resource *resource)
{
    Q_UNUSED(resource);
    delete this;
}
