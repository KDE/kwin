...
class XXWmPipV1InterfacePrivate : public QtWaylandServer::xx_wm_pip_v1
{
public:
    explicit XXWmPipV1InterfacePrivate(XXWmPipV1Interface *q, Display *display);

    XXWmPipV1Interface *q;
    Display *display;

protected:
    void xx_wm_pip_v1_destroy(Resource *resource) override;
    void xx_wm_pip_v1_get_pip(Resource *resource, uint32_t id, struct ::wl_resource *xdg_surface) override;
};

XXWmPipV1InterfacePrivate::XXWmPipV1InterfacePrivate(XXWmPipV1Interface *q, Display *display)
    : QtWaylandServer::xx_wm_pip_v1(*display, s_version)
    , q(q)
    , display(display)
{
}

void XXWmPipV1InterfacePrivate::xx_wm_pip_v1_destroy(Resource *resource)
{
    wl_resource_destroy(resource->handle);
}

void XXWmPipV1InterfacePrivate::xx_wm_pip_v1_get_pip(Resource *resource, uint32_t id, struct ::wl_resource *xdg_surface)
{
    ...
    if (const SurfaceRole *role = xdgSurface->surface()->role()) {
        ...
        xdgSurface->surface()->setRole(XdgToplevelInterface::role());
    }
...
XXWmPipV1Interface::XXWmPipV1Interface(Display *display, QObject *parent)
    : QObject(parent)
    , d(std::make_unique<XXWmPipV1InterfacePrivate>(this, display))
{
}

XXWmPipV1Interface::~XXWmPipV1Interface()
{
}

Display *XXWmPipV1Interface::display() const
{
    return d->display;
}
...
{
public:
    std::optional<QPointer<SurfaceInterface>> origin;
    std::optional<QRect> originRect;
};
...
{
public:
    XXPipV1InterfacePrivate(XXPipV1Interface *q, XXWmPipV1Interface *shell, XdgSurfaceInterface *xdgSurface);

    void apply(XXPipV1Commit *comit);
    void reset();

    XXPipV1Interface *q;
    XXWmPipV1Interface *shell;
...
XXPipV1InterfacePrivate::XXPipV1InterfacePrivate(XXPipV1Interface *q, XXWmPipV1Interface *shell, XdgSurfaceInterface *xdgSurface)
...
void XXPipV1InterfacePrivate::apply(XXPipV1Commit *commit)
{
    ...
    if (commit->origin.has_value()) {
        origin = commit->origin.value();
    }
    if (commit->originRect.has_value()) {
        originRect = commit->originRect.value();
    }
...
XXPipV1Interface::XXPipV1Interface(XXWmPipV1Interface *shell, XdgSurfaceInterface *xdgSurface, wl_resource *resource)
...
quint32 XXPipV1Interface::sendConfigure(const QSizeF &size)
{
...
void XXPipV1Interface::sendDismissed()
{
    d->send_dismissed();
}
