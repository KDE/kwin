...
protected:
    ...
    void xdg_toplevel_icon_v1_add_buffer(Resource *resource, struct ::wl_resource *icon, int32_t scale) override
    {
        ...
        if (view.isNull()) {
            wl_resource_post_error(resource->handle, XdgToplevelIconV1Interface::error_wrong_format, "Icon buffer invalid");
            return;
        }

        if (view.image()->width() != view.image()->height()) {
            wl_resource_post_error(resource->handle, XdgToplevelIconV1Interface::error_wrong_format, "Icon buffer is not a square");
            return;
        }

        QImage image = view.image()->copy();
        if (image.isNull()) {
            wl_resource_post_error(resource->handle, XdgToplevelIconV1Interface::error_wrong_format, "TopLevel icon invalid");
            return;
        }
