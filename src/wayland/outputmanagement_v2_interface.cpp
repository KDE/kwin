...
protected:
    ...
    void kde_output_configuration_v2_set_output_index(Resource *resource, wl_resource *output, uint32_t index) override;
};
...
void OutputConfigurationV2Interface::kde_output_configuration_v2_set_output_index(Resource *resource, wl_resource *outputResource, uint32_t index)
{
    if (invalid) {
        return;
    }
    if (OutputDeviceV2Interface *output = OutputDeviceV2Interface::get(outputResource)) {
        outputOrder.push_back(std::make_pair(index, output));
    }
}
