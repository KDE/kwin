...
protected:
    ...
    void kde_output_configuration_v2_set_dimming_multiplier(Resource *resource, ::wl_resource *outputdevice, uint32_t multiplier) override;
};
...
void OutputConfigurationV2Interface::kde_output_configuration_v2_set_dimming_multiplier(Resource *resource, ::wl_resource *outputdevice, uint32_t multiplier)
{
    if (invalid) {
        return;
    }
    if (OutputDeviceV2Interface *output = OutputDeviceV2Interface::get(outputdevice)) {
        config.changeSet(output->handle())->dimmingMultiplier = multiplier / 10'000.0;
    }
}
