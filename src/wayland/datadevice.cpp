    ...
    if (offer) {
        auto matchOffers = [this, dragSource, offer] {
            ...
            if (d->seat->isDrag()) { // ignore keyboard modifiers when in "ask" negotiation
                keyboardModifiers = input()->keyboardModifiers();
            }
        ...
        d->drag.keyboardModifiersConnection = connect(input(), &InputRedirection::keyboardModifiersChanged, dragSource, matchOffers);
    }
}
