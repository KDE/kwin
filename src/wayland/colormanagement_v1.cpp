...
void ColorManagerV1::wp_color_manager_v1_get_feedback_surface(Resource *resource, uint32_t id, struct ::wl_resource *surface)
{
    const auto surf = SurfaceInterface::get(surface);
    const auto priv = SurfaceInterfacePrivate::get(surf);
...
void ColorManagerV1::wp_color_manager_v1_new_icc_creator(Resource *resource, uint32_t obj)
{
    wl_resource_post_error(resource->handle, error::error_unsupported_feature, "ICC profiles are not supported");
}

void ColorManagerV1::wp_color_manager_v1_new_parametric_creator(Resource *resource, uint32_t obj)
{
...
ColorFeedbackSurfaceV1::ColorFeedbackSurfaceV1(wl_client *client, uint32_t id, uint32_t version, SurfaceInterface *surface)
    : QtWaylandServer::wp_color_management_feedback_surface_v1(client, id, version)
    , m_surface(surface)
...
void ColorFeedbackSurfaceV1::setPreferredColorDescription(const ColorDescription &descr)
{
    if (m_preferred != descr) {
        m_preferred = descr;
        send_preferred_changed(resource()->handle, 0);
    }
}

void ColorFeedbackSurfaceV1::wp_color_management_feedback_surface_v1_destroy_resource(Resource *resource)
{
    delete this;
}

void ColorFeedbackSurfaceV1::wp_color_management_feedback_surface_v1_destroy(Resource *resource)
{
    wl_resource_destroy(resource->handle);
}

void ColorFeedbackSurfaceV1::wp_color_management_feedback_surface_v1_get_preferred(Resource *resource, uint32_t image_description)
{
...
void ColorParametricCreatorV1::wp_image_description_creator_params_v1_create(Resource *resource, uint32_t image_description)
{
    ...
    if (*m_transferFunctionType == TransferFunction::PerceptualQuantizer) {
        func.minLuminance = 0;
        func.maxLuminance = 10'000;
    }
    const double minMasteringLuminance = m_minMasteringLuminance.value_or(func.minLuminance);
    if (m_maxCll && *m_maxCll < minMasteringLuminance) {
        wl_resource_post_error(resource->handle, error::error_invalid_luminance, "max_cll is larger than min. mastering luminance");
        return;
    }
    if (m_maxCll && *m_maxCll > m_maxMasteringLuminance.value_or(func.maxLuminance)) {
        wl_resource_post_error(resource->handle, error::error_invalid_luminance, "max_cll is larger than the max. mastering luminance");
        return;
    }
    if (m_maxFall && *m_maxFall < minMasteringLuminance) {
        wl_resource_post_error(resource->handle, error::error_invalid_luminance, "max_fall is larger than min. mastering luminance");
        return;
    }
    if (m_maxFall && *m_maxFall > m_maxMasteringLuminance.value_or(func.maxLuminance)) {
        wl_resource_post_error(resource->handle, error::error_invalid_luminance, "max_fall is larger than the max. mastering luminance");
        return;
    }
    if (m_maxFall && m_maxCll && *m_maxFall > m_maxCll) {
        wl_resource_post_error(resource->handle, error::error_invalid_luminance, "max_fall is larger than max_cll");
        return;
    }
    ...
    new ImageDescriptionV1(resource->client(), image_description, resource->version(), ColorDescription(*m_colorimetry, func, referenceLuminance, minMasteringLuminance, maxFrameAverageLuminance, maxHdrLuminance, m_masteringColorimetry, Colorimetry::fromName(NamedColorimetry::BT709)));
    wl_resource_destroy(resource->handle);
}
...
void ColorParametricCreatorV1::wp_image_description_creator_params_v1_set_tf_named(Resource *resource, uint32_t tf)
{
    ...
    case WP_COLOR_MANAGER_V1_TRANSFER_FUNCTION_ST2084_PQ:
        ...
        return;
    case WP_COLOR_MANAGER_V1_TRANSFER_FUNCTION_LINEAR:
...
static constexpr double s_primaryEncoding = 1'000'000.0;
...
void ColorParametricCreatorV1::wp_image_description_creator_params_v1_set_primaries(Resource *resource, int32_t r_x, int32_t r_y, int32_t g_x, int32_t g_y, int32_t b_x, int32_t b_y, int32_t w_x, int32_t w_y)
{
    ...
    m_colorimetry = Colorimetry{
        xy{r_x / s_primaryEncoding, r_y / s_primaryEncoding},
        xy{g_x / s_primaryEncoding, g_y / s_primaryEncoding},
        xy{b_x / s_primaryEncoding, b_y / s_primaryEncoding},
        xy{w_x / s_primaryEncoding, w_y / s_primaryEncoding},
    };
}
...
ImageDescriptionV1::ImageDescriptionV1(wl_client *client, uint32_t id, uint32_t version, const ColorDescription &color)
...
static uint32_t kwinTFtoProtoTF(TransferFunction tf)
{
    ...
    case TransferFunction::linear:
        return wp_color_manager_v1_transfer_function::WP_COLOR_MANAGER_V1_TRANSFER_FUNCTION_LINEAR;
...
void ImageDescriptionV1::wp_image_description_v1_get_information(Resource *qtResource, uint32_t information)
{
    ...
    const auto round = [](double f) {
        return std::clamp(std::round(f * s_primaryEncoding), 0.0, s_primaryEncoding);
    };
...
const ColorDescription &ImageDescriptionV1::description() const
{
