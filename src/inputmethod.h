 ...
 *
 * \brief Provides integration for input methods.
 *
 * Manages the compositor-side integration for zwp_input_method_unstable_v1, enabling an input
 * method to be used for text input in KWin, as well as the ability to display an on-screen keyboard
 * and overlay panels.
 */
...
public:
     ...
     * \value NonMouseInput
     *        The on-screen keyboard is shown automatically for non-mouse input devices
     *        (e.g., touch, stylus) when a text input field is focused.
     * \value AnyInput
     *        The on-screen keyboard is shown automatically when any input device (e.g., mouse,
     *        touch, stylus) focuses a text input field.
     */
    ...
    /*!
     * The input method is considered enabled if the on-screen keyboard is configured to show
     * automatically.
     */
    ...
    /*!
     * Considered active if the focused client has a Wayland text-input enabled and the input method
     ...
     *
     * This method is more of a "you are allowed to show yourself" than a "please show yourself"
     * request.
     */
    void show();
     ...
     *
     * If an input method process is already running, it will be terminated and the new command will
     * be used to launch a new process.
     */
    ...
    /*!
     * Whether the on-screen keyboard should be shown automatically when a text input field is
     * focused, based on the current visibility \l mode and the last input device used.
     */
    ...
    /*!
     * Whether the currently focused client supports Wayland's text-input protocol (v2 or v3) and
     * can receive text input from the input method.
     */
    bool activeClientSupportsTextInput() const;

    /*!
     * Request the input method to activate itself, even if the currently focused client does not
     * support text input.
     *
    ...
    /**
     * Injects text directly to the focused text-input; meant for sources other than the input
     * method (e.g., libei).
     */
...
private:
     ...
     */
    bool m_forceShowRequested = false;
};
