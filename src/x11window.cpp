...
void X11Window::blockGeometryUpdates(bool block)
{
    if (block) {
        ...
        if (--m_blockGeometryUpdates == 0) {
            ...
            const QRect xWrapperGeometry = Xcb::toXNative(m_clientGeometry.translated(-m_bufferGeometry.topLeft()));
            const QRect xClientGeometry = QRect(0, 0, xWrapperGeometry.width(), xWrapperGeometry.height());
            configure(xFrameGeometry, xWrapperGeometry, xClientGeometry);
        }
    }
}
...
void X11Window::configure(const QRect &frame, const QRect &wrapper, const QRect &client)
{
    if (m_frame.size() != frame.size() || m_wrapper.geometry() != wrapper) {
        if (m_frame.geometry() != frame) {
            m_frame.setGeometry(frame);
        }
        if (!isShade()) {
            if (m_wrapper.geometry() != wrapper) {
                m_wrapper.setGeometry(wrapper);
            }
            if (m_client.geometry() != client) {
                m_client.setGeometry(client);
            }
        ...
        updateInputShape();
        updateInputWindow();
    } else if (m_frame.position() != frame.topLeft()) {
        m_frame.move(frame.topLeft());
        if (m_decoInputExtent.isValid()) {
