...
QByteArray ShaderManager::generateFragmentSource(ShaderTraits traits) const
{
    ...
    if (traits & ShaderTrait::TransformColorspace) {
        ...
        stream << "const int normalizedLinear_EOTF = 4;\n";
        stream << "\n";
    ...
    if (traits & ShaderTrait::TransformColorspace) {
        ...
        stream << "    } else if (sourceNamedTransferFunction == normalizedLinear_EOTF) {\n";
        stream << "        result.rgb *= sdrBrightness;\n";
        stream << "    }\n";
    ...
    if (traits & ShaderTrait::TransformColorspace) {
        ...
        stream << "        result.rgb /= 80.0;\n";
        stream << "    } else if (destinationNamedTransferFunction == normalizedLinear_EOTF) {\n";
        stream << "        result.rgb /= sdrBrightness;\n";
        stream << "    }\n";
    }
