...
void ColorPickerEffect::postPaintScreen()
{
    ...
    if (m_scheduledPosition != QPoint(-1, -1) && (!m_paintedScreen || m_paintedScreen->geometry().contains(m_scheduledPosition))) {
        ...
        const QPoint texturePosition(screenPosition.x() * GLRenderTarget::virtualScreenScale(), (geo.height() - screenPosition.y() - PIXEL_SIZE) * GLRenderTarget::virtualScreenScale());
