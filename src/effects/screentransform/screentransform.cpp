...
static GLVertexBuffer *texturedRectVbo(const QRectF &geometry, qreal scale)
{
    ...
    auto scaled = scaledRect(geometry, scale);

    // first triangle
    map[0] = GLVertex2D{
        .position = QVector2D(scaled.left(), scaled.top()),
        .texcoord = QVector2D(0.0, 1.0),
    };
    map[1] = GLVertex2D{
        .position = QVector2D(scaled.right(), scaled.bottom()),
        .texcoord = QVector2D(1.0, 0.0),
    };
    map[2] = GLVertex2D{
        .position = QVector2D(scaled.left(), scaled.bottom()),
    ...
    map[4] = GLVertex2D{
        .position = QVector2D(scaled.right(), scaled.top()),
