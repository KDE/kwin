...
void MagnifierEffect::paintScreen(const RenderTarget &renderTarget, int mask, const QRegion &region, ScreenPaintData &data)
{
    ...
    if (m_zoom != 1.0) {
        ...
        if (effects->isOpenGLCompositing()) {
            m_fbo->blitFromFramebuffer(renderTarget.mapToRenderTarget(srcArea).toRect());
            // paint magnifier
            m_texture->bind();
