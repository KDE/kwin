...
void BlurEffect::drawWindow(const RenderTarget &renderTarget, EffectWindow *w, int mask, const QRegion &region, WindowPaintData &data)
{
    if (shouldBlur(w, mask, data)) {
        ...
        if (!shape.isEmpty()) {
            doBlur(renderTarget, shape, screen, data.opacity(), projectionMatrix, w->isDock() || transientForIsDock, w->frameGeometry().toRect());
        }
    }
...
void BlurEffect::doBlur(const RenderTarget &renderTarget, const QRegion &shape, const QRect &screen, const float opacity, const QMatrix4x4 &screenProjection, bool isDock, QRect windowRect)
{
    ...
    const QRect deviceSourceRect = scaledRect(logicalSourceRect, renderTarget.scale()).toRect();
    ...
    upscaleRenderToScreen(renderTarget, vbo, blurRectCount * (m_downSampleIterations + 1), shape.rectCount() * 6, screenProjection, windowRect.topLeft());
...
void BlurEffect::upscaleRenderToScreen(const RenderTarget &renderTarget, GLVertexBuffer *vbo, int vboStart, int blurRectCount, const QMatrix4x4 &screenProjection, QPoint windowPosition)
{
    m_renderTextures[1]->bind();

    m_shader->bind(BlurShader::UpSampleType);
    m_shader->setTargetTextureSize(m_renderTextures[0]->size() * renderTarget.scale());
