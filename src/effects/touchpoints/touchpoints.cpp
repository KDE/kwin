...
void TouchPointsEffect::paintScreen(const RenderTarget &renderTarget, int mask, const QRegion &region, ScreenPaintData &data)
{
    ...
    for (auto it = m_points.constBegin(), end = m_points.constEnd(); it != end; ++it) {
        for (int i = 0; i < m_ringCount; ++i) {
            ...
            if (size > 0 && alpha > 0) {
                ...
                drawCircle(renderTarget, color, it->pos.x(), it->pos.y(), size);
            }
        }
    }
...
void TouchPointsEffect::drawCircle(const RenderTarget &renderTarget, const QColor &color, float cx, float cy, float r)
{
...
void TouchPointsEffect::drawCircleGl(const RenderTarget &renderTarget, const QColor &color, float cx, float cy, float r)
{
