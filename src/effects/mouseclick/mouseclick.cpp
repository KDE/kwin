...
void MouseClickEffect::paintScreen(const RenderTarget &renderTarget, int mask, const QRegion &region, ScreenPaintData &data)
{
    ...
    for (const auto &click : m_clicks) {
        for (int i = 0; i < m_ringCount; ++i) {
            ...
            if (size > 0 && alpha > 0) {
                ...
                drawCircle(renderTarget, color, click->m_pos.x(), click->m_pos.y(), size);
            }
        }
        ...
        if (m_showText && click->m_frame) {
            ...
            click->m_frame->render(renderTarget, infiniteRegion(), frameAlpha, frameAlpha);
        }
    }
    for (const auto &tool : std::as_const(m_tabletTools)) {
        ...
        for (qreal size = m_ringMaxSize; size > 0; size -= step) {
            drawCircle(renderTarget, tool.m_color, tool.m_globalPosition.x(), tool.m_globalPosition.y(), size);
        }
    }
...
void MouseClickEffect::drawCircle(const RenderTarget &renderTarget, const QColor &color, float cx, float cy, float r)
{
    if (effects->isOpenGLCompositing()) {
        drawCircleGl(renderTarget, color, cx, cy, r);
...
void MouseClickEffect::drawCircleGl(const RenderTarget &renderTarget, const QColor &color, float cx, float cy, float r)
{
    ...
    const float scale = renderTarget.scale();
    float t;
