...
void ContrastEffect::uploadRegion(QVector2D *&map, const QRegion &region, qreal scale)
{
    for (const QRect &r : region) {
        ...
        const QVector2D topLeft(scaled.x(), scaled.y());
        const QVector2D topRight(scaled.x() + scaled.width(), scaled.y());
        const QVector2D bottomLeft(scaled.x(), scaled.y() + scaled.height());
        const QVector2D bottomRight(scaled.x() + scaled.width(), scaled.y() + scaled.height());
