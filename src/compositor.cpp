...
static std::optional<std::unordered_map<OutputLayer *, Item *>> assignLayers(RenderView *sceneView, std::span<Item *const> items, std::span<OutputLayer *const> layers)
{
    ...
    for (; itemIt != items.end();) {
        ...
        if (!recommendedSizes.isEmpty()) {
            ...
            const QRectF sceneRect = compositingAllowed ? item->mapToView(item->boundingRect(), sceneView) : item->mapToView(item->rect(), sceneView);
...
std::pair<QList<Compositor::LayerData>, bool> Compositor::setupLayers(SceneView *sceneView, Output *output,
                                                                      ...
                                                                      std::unordered_set<OutputLayer *> &toUpdate)
{
    ...
    for (auto it = sortedLayers.begin(); it != sortedLayers.end(); it++) {
        ...
        if (isScene) {
            ...
            m_overlayViews[output->renderLoop()].erase(layer);
        ...
        if (!view || view->item() != item) {
            if (isCursor) {
                ...
                view = std::make_unique<ItemTreeView>(sceneView, item, output, layer);
                connect(layer, &OutputLayer::repaintScheduled, view.get(), [output, cursorView = view.get()]() {
                ...
                view = std::make_unique<ItemView>(sceneView, item, output, layer);
            }
        }
        layers.push_back(LayerData{
            .view = view.get(),
    ...
    for (auto &layer : layers) {
        if (type != SetupType::Fallback && prepareDirectScanout(layer.view, output, frame)) {
    ...
    return std::make_pair(layers, output->testPresentation(frame));
}
...
void Compositor::composite(RenderLoop *renderLoop)
{
    ...
    const QList<Item *> overlayCandidates = m_scene->overlayCandidates(allowedOutputLayers.size());
    auto idealLayerAssignments = assignLayers(primaryView, overlayCandidates, allowedOutputLayers);

    // fallback 1
    auto scenePlusCursor = overlayCandidates | std::views::filter([](Item *item) {
    ...
    std::unordered_set<OutputLayer *> toUpdate;
    auto [layers, result] = setupLayers(primaryView, output, allowedOutputLayers, *idealLayerAssignments,
    ...
    if (!result) {
        ...
        if (!fallback1) {
            idealLayerAssignments = assignLayers(primaryView, scenePlusCursor, allowedOutputLayers);
            std::tie(layers, result) = setupLayers(primaryView, output, allowedOutputLayers, *idealLayerAssignments,
