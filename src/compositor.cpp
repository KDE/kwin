...
static bool prepareDirectScanout(RenderView *view, Output *output, const std::shared_ptr<OutputFrame> &frame)
{
    ...
    const auto geometry = candidate->mapToScene(QRectF(QPointF(0, 0), candidate->size()), view->scale()).translated(-output->geometryF().topLeft());
...
{
    if (layers.empty() || items.empty() || s_disableOverlays) {
        return {};
    }
    ...
    for (; itemIt != items.end();) {
        ...
        const QRectF sceneRect = item->mapToScene(item->rect(), sceneView->scale());
...
void Compositor::composite(RenderLoop *renderLoop)
{
    ...
    if (!m_brokenCursors[renderLoop] && cursorItem->isVisible() && cursorItem->mapToScene(cursorItem->boundingRect(), primaryView->scale()).intersects(output->geometryF())) {
