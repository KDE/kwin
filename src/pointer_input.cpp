...
void PointerInputRedirection::processStrokeGestureBegin(const std::vector<QPointF> &points, std::chrono::microseconds time, KWin::InputDevice *device)
{
    input()->setLastInputHandler(this);
    if (!inited()) {
        return;
...
void PointerInputRedirection::processStrokeGestureUpdate(const std::vector<QPointF> &points, std::chrono::microseconds time, KWin::InputDevice *device)
{
    input()->setLastInputHandler(this);
    if (!inited()) {
        return;
