...
void AutoBrightnessCurve::adjust(double brightness, double lux)
{
    ...
    // unitless factor
    constexpr double minRelativeDifference = 1.1;
    ...
    //      - m_luxAtBrightness[i] >= m_luxAtBrightness[i - 1] * minRelativeDifference
    //          - aka m_luxAtBrightness[i] / minRelativeDifference >= m_luxAtBrightness[i - 1]
    ...
    if (lowMatch == m_luxAtBrightness.size() - 1) {
        ...
        for (int i = m_luxAtBrightness.size() - 2; i >= 0; i--) {
            m_luxAtBrightness[i] = std::min({m_luxAtBrightness[i], last / minRelativeDifference, last - minAbsDifference});
    ...
    // We need to enforce "m_luxAtBrightness[i] >= m_luxAtBrightness[i - 1] * minRelativeDifference",
    ...
    // which ensures "(high - low) >= minRelativeDifference * lux", which is good enough
    const double minHighValueRelative = lux + (minRelativeDifference - 1) * (1 - highFactor) * lux;
    ...
    for (size_t i = lowMatch + 2; i < m_luxAtBrightness.size(); i++) {
        m_luxAtBrightness[i] = std::max({m_luxAtBrightness[i], last * minRelativeDifference, minAbsDifference});
        last = m_luxAtBrightness[i];
    }
}
