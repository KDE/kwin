...
ColorPipeline ColorPipeline::create(const std::shared_ptr<ColorDescription> &source, const IccProfile *destination, RenderingIntent intent, InputType inputType)
{
    ...
    if (const auto tag = destination->BToATag(intent)) {
        ...
        if (intent == RenderingIntent::AbsoluteColorimetricNoAdaptation) {
            ...
            ret = create(source, linearizedProfile, RenderingIntent::AbsoluteColorimetricNoAdaptation, inputType);
            ...
            ret = create(source, iccConnectionSpace, intent, inputType);
        }
        ...
        ColorPipeline ret = create(source, linearizedProfile, intent, inputType);
